...
- In `subtrees` release mode, treat `socket` like a standard protected-main release: validate, branch or PR when needed, clear CI and PR comments before tagging, merge to `main`, fast-forward local `main`, tag the superproject from reviewed `main`, push the tag, create the GitHub release, run `codex plugin marketplace upgrade socket`, try the best-effort Mac mini marketplace refresh over SSH, and only add the subtree accounting gates that determine whether each child repo needs pull or push.
...
Expect occasional patch-only Socket releases whose practical purpose is to make Codex refresh Git-backed plugin entries exposed through the Socket marketplace, especially `speak-swiftly` from the standalone `gaelic-ghost/SpeakSwiftlyServer` source. These are still real Socket releases, not cache-only chores: bump the shared version surfaces, validate, run the release-ready gate, satisfy any subtree accounting, tag and publish the Socket release, and only then run `codex plugin marketplace upgrade socket` plus the best-effort Mac mini marketplace refresh.
