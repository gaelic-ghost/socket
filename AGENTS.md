...
- `apple-dev-skills` and `python-skills` are the only child repositories here that still keep explicit upstream subtree sync paths.
...
- Start from the root docs when the task is about the mixed monorepo model, root marketplace wiring, subtree sync for `apple-dev-skills` or `python-skills`, or superproject release flow.
...
- For `apple-dev-skills` and `python-skills`, keep subtree sync operations explicit:
