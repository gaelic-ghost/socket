...
- [x] Add `productivity-skills:dice-job-search-workflow` after verifying Dice's official MCP docs and setup pages. Keep the first pass guidance-only around Dice's remote `search_jobs` MCP tool, with explicit authentication, rate-limit, saved-search, application-state, and privacy boundaries.
