...
- [ ] Investigate a Dice.com MCP server skill after the newly launched Dice webdocs can be verified from an authoritative source. Decide whether the work belongs in a job-search/productivity plugin, a broader career-search workflow, or a standalone MCP server skill, and require explicit authentication, rate-limit, saved-search, application-state, and privacy boundaries before any implementation plan.
