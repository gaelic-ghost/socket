...
- [ ] Expand Apple Dev Skills with dedicated SwiftUI animation, Core Animation, SF Symbols, and Apple typography workflows. First slice shipped `sf-symbols-workflow` and `swiftui-animation-workflow`; next slices should add Core Animation and Apple typography from [`docs/maintainers/apple-design-animation-skills-plan.md`](./docs/maintainers/apple-design-animation-skills-plan.md), keep the skills under `plugins/apple-dev-skills`, use Xcode-local docs, Dash, official Apple docs, and local Apple developer apps as evidence, and avoid absorbing this work into the existing SwiftUI architecture skill.
