...
- [ ] Grow the first Swift Steward subagent role from the draft custom-agent files and [`docs/agents/swift-steward-subagent-plan.md`](./docs/agents/swift-steward-subagent-plan.md) into a verified plugin-facing workflow. Start with read-heavy guidance-sync and repo-maintenance scans, then add reviewable draft patch output before any apply-mode behavior.
