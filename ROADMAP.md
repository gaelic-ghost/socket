...
- [ ] Explore an `ErrorHandles` Swift helper package for consistent, concise error construction, diagnostic context, typed wrapping, and recovery helpers. Discuss whether generics, variadic generics or parameter packs, and macros would make the API meaningfully cleaner before creating a package or adding local helper frameworks.
