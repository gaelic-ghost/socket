...
- [ ] Expand the plugin as composable shared-analysis, domain, and thin tool-adapter skills rather than one Apple or tool catch-all.
- [ ] Add platform and tool workflows after representative artifacts have calibrated Cutter/Rizin, Malimite, Ghidra, Hopper, and adjacent tools.
...
- [ ] Add shared `select-analysis-path`, `review-decompiler-output`, `preserve-binary-artifacts`, and `compare-binary-versions` workflows.
...
- [ ] Shared analysis, Apple domain, tool-adapter, Unity, and .NET workflows each have a clear owner skill or an explicit reason to stay backlog-only.
- [ ] Stable Apple guidance remains usable without beta facts, while beta and hardware-sensitive claims require exact build, hardware, tool, and source evidence.
- [x] Root Socket docs, marketplace wiring, and validation agree on the plugin's install surface.
