...
- [ ] Split the `productivity-skills` catch-all into `repository-skills`, `documentation-skills`, `professional-skills`, and `agent-engineering-skills`. Start with the durable [agent skills domain split plan](./docs/maintainers/agent-skills-domain-split.md): Agent Engineering Skills owns the coordinator/worker and external-agent contracts; language plugins retain runtime implementation; Agent Portability Skills retains host adapters; and `productivity-skills` is retired only after every skill, role, test, manifest, marketplace entry, and install reference has a clear new owner.
