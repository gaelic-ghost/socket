...
When the user explicitly requests subagents, or applicable workflow guidance tells the agent to ask and the user grants explicit permission, this skill can split read-heavy discovery for large code slices into bounded subagent tasks. Good subagent jobs include mapping call sites, reading tests, checking docs, or tracing one branch of a comparison, with each worker returning concise file references and findings.
