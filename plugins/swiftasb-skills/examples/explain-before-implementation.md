...
What SwiftASB would do here: SwiftASB would let the SwiftUI app start the local Codex runtime, open a thread for the workspace, start turns, read app-server-owned workspace and worktree facts, and read live progress through Swift-native handles instead of replaying raw app-server JSON.
...
What SwiftASB would own: SwiftASB owns app-server startup, app-server-owned fact reads, worktree snapshots, thread and turn handles, diagnostics, interactive request routing, query descriptors, and local history helpers.
