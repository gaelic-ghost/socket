...
- SwiftASB `v1.0.0` is the first supported public API baseline, and `v1.1.4` is the current released baseline for app-wide library, project identity, thread source, filesystem, config, extension-inventory, MCP-resource, diagnostics, workspace-permission, query-descriptor, thread-management, and recent-activity guidance; verify local or GitHub package state before writing exact API guidance.
