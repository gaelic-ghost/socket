...
- When applying an upstream Runpod skill that suggests a shell wait loop for a remote build, provision, deployment, or service startup, do not reproduce that loop in agent execution. Record the concrete target and state, reuse a live matching host-native continuation while the gate is pending and healthy, and inspect on wakeup. Continue only when the recorded Runpod target and state exactly match the fresh state; otherwise stop and create a new continuation packet. Do not delete/recreate it for an unchanged snapshot; create/update only after it fires or becomes stale, with at least five minutes between rechecks. Codex uses heartbeat; Hermes uses a continuable `cronjob` with `deliver="origin"` and `attach_to_session=true`. Keep this as an execution overlay until the upstream mirror can be refreshed with equivalent source guidance.
