...
- Treat `xcode-select` as the only default selector for Xcode command-line tools. Run `xcodebuild`, `xcrun`, `swift`, Metal tools, and related CLI commands through Gale's currently selected developer directory without overriding it. Do not set `DEVELOPER_DIR`. If it is genuinely the only possible way to complete a task, explain why `xcode-select` cannot serve that task and obtain Gale's explicit permission before using it. When Gale asks to change the selected toolchain, use `xcode-select --switch`, verify with `xcode-select -p` and tool versions, and leave the selected state exactly as Gale requested.
