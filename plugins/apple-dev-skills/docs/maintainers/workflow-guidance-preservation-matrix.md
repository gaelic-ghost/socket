...
| Manifest edits, dependency changes, targets, products, and plugins | `swift-package-workflow/SKILL.md` and `references/cli-command-matrix.md` | `swift-package-build-run-workflow` | yes, for the durable “edit `Package.swift` intentionally, avoid unnecessary branch/revision requirements, and treat generated dependency-state files as generated” rule | Build/run owns the operational shape changes, but package `AGENTS.md` should still carry the lasting policy that `Package.swift` may be edited intentionally, `Package.resolved` should not be hand-edited, and related package graph updates should stay consolidated when practical. |
