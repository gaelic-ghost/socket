...
class XcodeGuidanceSyncWorkflowTests(unittest.TestCase):
    ...
    def test_sync_creates_agents_template(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("three-letter uppercase prefix", agents_text)
            self.assertIn("do not use external ViewModels as a SwiftUI shape", agents_text)
            self.assertIn("Sources/Views/Shared", agents_text)
            self.assertIn("`Internal/` for app-owned feature services", agents_text)
            self.assertIn("Do not create an umbrella `GEAAppService.swift`", agents_text)
            self.assertNotIn("+ViewModel.swift", agents_text)
    ...
    def test_sync_appends_section_to_existing_agents(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Prefer enums, enum cases with associated values", agents_text)
            self.assertIn("three-letter uppercase prefix", agents_text)
            self.assertIn("Do not create external SwiftUI ViewModel files", agents_text)
            self.assertIn("Sources/Views/Shared", agents_text)
            self.assertIn("`Internal/` for app-owned feature services", agents_text)
            self.assertIn("Do not add an umbrella `WhateverNameAppService.swift`", agents_text)
            self.assertNotIn("+ViewModel.swift", agents_text)
            self.assertTrue(Path(tmpdir, ".swiftformat").is_file())
            self.assertTrue(Path(tmpdir, "Scripts/repo-maintenance/hooks/pre-commit.sample").is_file())
