...
class SwiftPackageGuidanceSyncWorkflowTests(unittest.TestCase):
    ...
    def test_sync_creates_agents_template(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("normal SwiftPM parallel test execution", agents_text)
            self.assertIn("unload_models", agents_text)
            self.assertIn("reload_models", agents_text)
            ...
            self.assertIn("swift-configuration", agents_text)
            self.assertIn("Reloading", agents_text)
            self.assertIn("YAML", agents_text)
            self.assertIn("CommandLineArguments", agents_text)
            self.assertIn("PropertyList", agents_text)
            self.assertIn("Logging", agents_text)
            self.assertIn("SwiftLog.Logger", agents_text)
            self.assertIn("Configuration", agents_text)
            self.assertIn("runtime UI accessibility verification", agents_text)
            self.assertIn("streamlined, compact, ergonomic call sites", agents_text)
            self.assertIn("optional parameters with explicit default values", agents_text)
    ...
    def test_sync_discovers_socket_cache_sibling_productivity_runner(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            apple_skill = cache_root / "apple-dev-skills" / "6.5.1" / "skills" / "sync-swift-package-guidance"
            productivity_skill = (
                cache_root
                / "productivity-skills"
                / "6.5.1"
            ...
            repo_root.mkdir()
            (repo_root / "Package.swift").write_text("// swift-tools-version: 6.3\n", encoding="utf-8")
