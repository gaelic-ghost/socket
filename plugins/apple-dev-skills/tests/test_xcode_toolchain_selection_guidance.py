    ...
    def test_xcode_workflows_require_xcode_select_and_permission_for_exceptions(self) -> None:
        ...
        for reference in references:
            with self.subTest(reference=reference):
                ...
                self.assertIn("Xcode > Settings > Locations", text)
                self.assertIn("Command Line Tools", text)
                self.assertIn("Touch ID or administrator prompt", text)
                self.assertNotIn("sudo xcode-select --switch /Applications/Xcode.app/Contents/Developer", text)
                self.assertNotIn("sudo xcode-select --switch /Applications/Betas/Xcode-beta.app/Contents/Developer", text)
                self.assertIn("verify with `xcode-select -p`", text)
