...
"""
        with tempfile.TemporaryDirectory() as tmpdir, fake_tools_in_path(("xcodegen", xcodegen_body), ("xcodebuild", xcodebuild_body)) as env:
            ...
            self.assertFalse((target / "Sources" / "Views" / "Shared" / "GEAContentViewModel.swift").exists())
            self.assertFalse((target / "Sources" / "Services" / "Internal" / "GEAAppService.swift").exists())
