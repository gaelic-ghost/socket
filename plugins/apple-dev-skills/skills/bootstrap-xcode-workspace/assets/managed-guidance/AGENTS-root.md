...
- `Apps/` owns platform app and test targets, `Packages/` owns shared SwiftPM
  modules, and `Services/` owns deployable SwiftPM executables. Edit sources,
  specs, `.xcconfig` files, and `Package.swift`; never
  hand-edit generated `.pbxproj` data.
- Run `just align` after updating Socket-managed guidance or project sources.
- The nested `Apps/AGENTS.md`, `Packages/AGENTS.md`, and `Services/AGENTS.md`
  add scoped guidance and inherit this file.
