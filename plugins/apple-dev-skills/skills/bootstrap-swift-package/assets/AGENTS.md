...
- Keep `swift-configuration` as the default configuration dependency for new packages unless the package has a concrete reason to remove it. The generated manifest should depend on `https://github.com/apple/swift-configuration` from `1.2.0`, enable the `.defaults`, `Reloading`, and `YAML` package traits, and add the `Configuration` product to the primary target.
...
- Keep package manifests on the explicit Swift 6 language-mode default by preserving `swiftLanguageModes: [.v6]` unless the user asks for a narrower language-mode contract, and treat the generated `// swift-tools-version:` as a starting point that may be lowered to match the real package compatibility target as long as it stays at `6.1` or newer while trait-enabled dependencies remain in the manifest.
