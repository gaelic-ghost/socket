...
load_swift_version() {
  ...
  if [[ -z "$swift_version_output" ]]; then
    blocked "Unable to determine the local Swift toolchain version. The bootstrap skill requires Swift 6.2 or newer because the default swift-configuration dependency uses PackageDescription package traits."
  fi
  ...
  blocked "Unable to parse the local Swift toolchain version from 'swift --version'. The bootstrap skill requires Swift 6.2 or newer because the default swift-configuration dependency uses PackageDescription package traits."
}
...
ensure_supported_swift_toolchain() {
  ...
  if (( swift_major_version < 6 )); then
    blocked "Swift $swift_major_version.$swift_minor_version is too old for this bootstrap workflow. The supported floor is Swift 6.2 or newer because the default swift-configuration dependency uses PackageDescription package traits."
  fi
