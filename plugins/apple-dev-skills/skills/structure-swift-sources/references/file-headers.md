...
- Use `scripts/normalize-swift-structure.fsx` to audit header presence and shape across `.swift` files.
- Start from `references/file-header-inventory.template.yaml` when you want a user-editable inventory file for `--apply` mode.
- Use `scripts/normalize-swift-structure.fsx --apply --inventory <yaml>` when you already have explicit `Purpose` and `Concern` text for each file and want the script to normalize placement and formatting deterministically.
...
```bash
scripts/normalize-swift-structure.fsx --apply --inventory path/to/headers.yaml
```
