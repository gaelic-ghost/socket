...
- For Apple, Swift, SwiftPM, and Xcode documentation, use Xcode MCP DocumentationSearch first, then the Dash.app MCP when its installed docsets cover the question. Use Dash HTTP only when the Dash.app MCP is unavailable or incomplete; consult GitHub/source repositories and readable online docs only after those local MCP paths.
