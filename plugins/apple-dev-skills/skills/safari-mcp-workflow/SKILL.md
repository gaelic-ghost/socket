...
2. Preflight Safari Technology Preview 247 or later, its `safaridriver` executable, and Safari's **Enable remote automation and external agents** setting. Read `references/setup-and-privacy.md`.
...
4. Establish page state with tab, URL, loading, and `get_page_content` evidence before evaluating JavaScript or taking a screenshot. Then collect the smallest useful combination of console, network, DOM, JavaScript, and screenshot evidence.
5. Perform only the authorized interaction sequence; batch related reversible steps through `page_interactions` when that makes the recorded sequence clearer. Do not submit forms, purchase, delete, change account state, or reveal secrets without a fresh user confirmation immediately before the effect.
