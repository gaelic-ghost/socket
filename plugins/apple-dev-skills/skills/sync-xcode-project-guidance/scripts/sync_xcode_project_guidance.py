...
def maintain_project_repo_runner() -> Path:
    ...
    raise RuntimeError(
        "sync-xcode-project-guidance needs repository-skills/maintain-project-repo "
...
def main() -> int:
    ...
    try:
        ...
        payload = {
            ...
            "actions": actions,
            "stderr": str(exc),
            "next_step": "Install repository-skills alongside apple-dev-skills, or add the socket marketplace and enable both plugin entries from the Socket catalog, then rerun sync-xcode-project-guidance.",
        }
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 1
