...
| --- | --- | --- |
| `codesign -dvvv --entitlements :- <artifact>` | identity, signature details, and embedded entitlements | Inspection only; do not infer distribution readiness from one field. |
| `codesign --verify --deep --strict --verbose=2 <artifact>` | signature integrity across nested code | Diagnose nested signing before re-signing. |
| `spctl -a -vv <artifact>` | Gatekeeper assessment | Keep local execution and trust-policy results separate. |
