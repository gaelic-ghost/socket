...
def specialized_handoff(operation_type: str, package_context: dict, request: str | None) -> tuple[str | None, str | None]:
    text = normalize_request_text(request)
    ...
    if operation_type == "test" and package_context["xctestplans"] and "test plan" in text:
...
def routing_summary(operation_type: str, package_context: dict, request: str | None) -> str:
    resource_focused = request_mentions_resources(request)
    target = package_context["source_targets"][0] if len(package_context["source_targets"]) == 1 else None
    if operation_type == "test":
...
def main() -> int:
    ...
    payload = {
        ...
        "output": {
            ...
            "routing_summary": routing_summary(operation_type, package_context, args.request),
            "inferred_context": {
