...
- If Xcode MCP docs fail or are unavailable, record the concrete failure reason plainly, then fall back through Dash MCP, Dash localhost HTTP, open source Swift repositories or generated docs when applicable, and readable official web docs in that order.
- Do not replace a failed Xcode MCP lookup with generic no-JS web search against Apple Developer pages. Use a real readable source or say the Apple docs could not be read in the current tool surface.
