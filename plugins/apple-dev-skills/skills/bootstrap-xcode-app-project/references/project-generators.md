...
- Use by default for new Xcode app and framework projects unless the user explicitly prefers the standard Xcode project-creation flow or the repository has a concrete reason to avoid a generator dependency. Use `bootstrap-xcode-workspace` to compose several generated app projects into a workspace.
...
- The generated scaffold should require the recent validated XcodeGen baseline declared in the template. As of this guidance, that baseline is `minimumXcodeGenVersion: 2.46.0`.
...
- Generate the standard top-level app repo layout: `Sources/`, `Tests/`, `Shared/`, `Extensions/`, `Configurations/`, `Scripts/`, and `Packages/`.
- Use exactly one top-level `Sources` entry for the app target, exactly one top-level `Shared` entry for shared app/extension source, and exactly one top-level `Tests` entry for the test target. Extension targets use one `Extensions/<ExtensionName>` source entry per extension target.
...
- When an app project participates in an `.xcworkspace`, set `options.schemePathPrefix: "../"` for workspace-relative scheme file paths; retain XcodeGen's standalone-project default otherwise.
