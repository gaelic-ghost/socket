...
- Use Xcode MCP DocumentationSearch or local Apple documentation first for Apple-owned SDK, framework, and lifecycle questions; use Dash MCP or Dash HTTP next when installed local package docs or multi-ecosystem docs are a better fit; then use official Apple documentation when local docs are insufficient.
