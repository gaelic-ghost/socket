...
- Treat XcodeGen as per-project generation. In an `Apps/Packages/Services` workspace, each app project declares its own local Swift packages; a workspace makes projects convenient to open together but does not replace target dependency wiring.
...
- Preserve the canonical Swift product workspace when adding or moving generated targets: app, extension, and Xcode test target peers under `Apps/`; reusable SwiftPM modules under `Packages/`; deployable server executables under `Services/`; and shared configuration and automation under `Configurations/` and `Scripts/`.
...
- Do not fragment ordinary XcodeGen source roots by subdirectory. Each app, extension, or Xcode test target gets one broad `Apps/<Target>` source entry that contains its source, resources, support files, generated plist, entitlements, and nested feature folders. Record and embed every extension from its explicit host target; never create a root `Extensions/` directory.
