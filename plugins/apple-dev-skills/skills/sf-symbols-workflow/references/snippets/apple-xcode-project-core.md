...
- Use the canonical product workspace layout for every new or adopted Swift product repository: `Apps/`, `Packages/`, `Services/`, `Configurations/`, and `Scripts/`, with one root `.xcworkspace` as the human and automation entrypoint.
- Put each application, app extension, and Xcode test target in its own peer directory under `Apps/<Target>/`. Record every extension's host app explicitly in project metadata and embed it from that host; never create a root `Extensions/` directory.
- Put reusable SwiftPM modules under `Packages/<Package>/` and deployable server executables under `Services/<Service>/`. A repository may contain any combination of these components without changing repository type or converting its entrypoint.
- Keep those top-level roots stable. Do not invent parallel roots such as `Sources/`, `Tests/`, `Shared/`, `Extensions/`, `AppSources`, `TestSources`, `Config`, `BuildScripts`, or `LocalPackages` for product-wide ownership. Component-local SwiftPM `Sources/` and `Tests/` directories remain standard inside `Packages/<Package>/` and `Services/<Service>/`.
- Inside an app target's source root, use this strict structure by default: `Views/`, `Models/`, and `Services/`. Do not create a `Controllers/` directory.
- The target's `Views/` directory owns SwiftUI views and UIKit/AppKit view surfaces. Use `Views/Shared`, `Views/macOS`, and `Views/iOS` so shared, macOS-specific, and iOS/iPadOS-specific UI have clear homes.
...
- For new Xcode app and framework projects, prefer an XcodeGen-backed project by default unless the user explicitly asks for a hand-managed Xcode project or the repository has a concrete reason to avoid a generator dependency. For a modular workspace, use `bootstrap-xcode-workspace`: XcodeGen owns each generated app project while Xcode owns the workspace composition.
...
- Do not fragment ordinary XcodeGen source roots by subdirectory. Each app, extension, or Xcode test target gets one broad `Apps/<Target>` source entry containing its source, resources, support files, generated plist, entitlements, and nested feature folders. If a project has another separate component-local logical root, use one entry for that root, not one entry per child folder.
...
- In an `Apps/Packages/Services` workspace, declare shared Core modules as local Swift package products in each consuming app project's top-level `packages` map, then reference each required product from that app target's `dependencies`. A package declaration alone does not link code into a target. Use `projectReferences` only for genuine dependencies on another Xcode project's target; neither a group nor a folder reference is a module boundary.
- For workspace project schemes with workspace-relative file paths, set `schemePathPrefix: "../"`; retain XcodeGen's standalone default when no workspace owns the scheme.
