...
- In an Apps/Packages workspace, declare shared Core modules as local Swift package products in each consuming app project. Use `projectReferences` only for genuine dependencies on another Xcode project's target; neither a group nor a folder reference is a module boundary.
