# Swift Package Testing Workflow Customization Contract

## Purpose

Document the fixed SwiftPM-first policy defaults for package testing and Xcode handoff behavior.
...
- `scripts/run_workflow.py` still loads customization state, but the current workflow uses fixed SwiftPM-first testing defaults rather than ordinary user-facing customization knobs.
...
2. Update `SKILL.md` and the affected workflow references to reflect the approved package-testing policy change.
...
5. Verify `scripts/run_workflow.py --operation-type test --dry-run` still emits the fixed SwiftPM-first testing workflow defaults.

## Validation

1. Verify the docs still describe a SwiftPM-first testing workflow.
