...
- Use `CVPixelBuffer`, `CVPixelBufferPool`, Core Video image-buffer attachments, `CVMetalTextureCache`, `CVMetalTexture`, and IOSurface-backed storage for frame pixels, allocation provenance, planes, color/signal properties, and hardware/GPU interoperability.
- Use `VTCompressionSession`, `VTDecompressionSession`, `VTSession`, `VTMultiPassStorage`, VideoToolbox property keys, encode/decode info flags, callback status, and `OSStatus` for low-level codec sessions and evidence.
...
- Choose Core Video when the bug or design turns on pixel-buffer storage, planes, pools, attachments, IOSurface compatibility, or Metal texture-cache interoperability.
- Choose VideoToolbox only when AVFoundation cannot express the required low-level codec, hardware-selection, latency, per-frame, multipass, property, or callback control.
...
- Do not replace `CVPixelBuffer`, its pool, planes, pixel format, attachments, IOSurface provenance, or Metal texture relationship with a raw byte wrapper while Apple media APIs still own the frame.
- Do not hide VideoToolbox sessions, supported properties, callback status/info flags, delayed-frame lifecycle, or `OSStatus` behind a generic codec interface before the actual codec behavior is diagnosed.
