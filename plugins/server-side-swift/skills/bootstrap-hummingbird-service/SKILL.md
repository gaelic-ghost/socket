...
description: Bootstrap new Hummingbird server-side Swift services with the official hb CLI, Hummingbird configuration support, Fluent ORM, PostgreSQL, Docker Compose local dependencies, generated AGENTS guidance, and SwiftPM validation. Use when creating a fresh Hummingbird service from scratch or maintaining this skill's bootstrap defaults.
...
The practical decision is not whether Hummingbird should use Fluent, direct Postgres, or a hand-built SwiftPM scaffold. The default is already chosen: start with the official `hb` CLI, keep Hummingbird's generated application shape, use Hummingbird configuration support for runtime settings, add Fluent ORM with PostgreSQL, and provide a Docker Compose Postgres service for local development.
...
- Do not substitute a manual SwiftPM scaffold, the Hummingbird template repository, direct Postgres-only persistence, SQLite, or non-container local database setup unless the user explicitly approves that exception for the project.
...
3. Create the service with the official CLI:
   ...
   hb init <name>
   ```
4. Answer the CLI prompts using the service's intended shape. Preserve Hummingbird's generated application and executable layout instead of replacing it with a hand-written SwiftPM scaffold.
...
7. Add local database container support:
   - create `compose.yaml` with a `postgres` service
