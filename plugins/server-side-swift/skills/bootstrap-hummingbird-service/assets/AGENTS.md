...
- This repository is a Hummingbird server-side Swift service created with the official `hb` CLI. Preserve the CLI-selected Server or Lambda application shape unless Gale explicitly asks for a migration.
- Keep Swift Package Manager as the source of truth for package structure, dependencies, builds, tests, and run commands.
- Use Hummingbird's generated `swift-configuration` support for runtime settings. Do not add a parallel bespoke settings loader unless Gale explicitly approves that design for this service.
- Default persistence for long-running Server apps is Fluent ORM with PostgreSQL. Do not replace it with direct Hummingbird Postgres, PostgresNIO, SQLite, or in-memory persistence unless Gale explicitly approves the project-specific exception.
- For Lambda apps generated by `hb`, keep `hummingbird-lambda` as the Lambda adapter. When OpenAPI is selected, keep `OpenAPIHummingbird` as the transport that registers generated handlers on the Hummingbird router.
...
- For generated Lambda apps, the executable owns Lambda startup and adapter setup instead of a long-running server process.
