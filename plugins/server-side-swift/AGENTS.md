...
- Use `bootstrap-hummingbird-service` for fresh Hummingbird services. Fresh Hummingbird services must start with the official `hb` CLI, preserve the CLI-selected Server or Lambda application shape, use Hummingbird's built-in or generated `swift-configuration` support, default long-running services to Fluent ORM with PostgreSQL, and include Docker Compose local PostgreSQL support unless Gale explicitly approves a project-specific exception.
- Use `sync-hummingbird-service-guidance` when an existing Hummingbird repository needs its repo-local `AGENTS.md`, Codex local environment file, command guidance, or `hb`-generated Server, Lambda, OpenAPI, Docker, and configuration assumptions brought in line with the current Socket baseline.
