...
- Prefer the official Hummingbird `hb` CLI for fresh Hummingbird app creation when current Hummingbird docs and the user request fit that path. Treat the Hummingbird template repository as fallback, source inspection, or explicit-template guidance rather than the first default.
