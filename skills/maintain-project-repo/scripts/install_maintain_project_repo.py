...
PROFILE_OVERLAY_FILES = {
    ...
    "xcode-workspace": [
        ...
        (
            "profiles/xcode-workspace/repo-maintenance/workspace/validate-components.sh",
            "scripts/repo-maintenance/workspace/validate-components.sh",
        ),
    ],
}
...
def xcode_workspace_findings(repo_root: Path) -> list[str]:
    findings: list[str] = []
    workspaces = list(repo_root.glob("*.xcworkspace"))
    if len(workspaces) != 1:
        findings.append(f"expected exactly one root .xcworkspace, found {len(workspaces)}")
...
def ensure_profile_shape(repo_root: Path, profile: str) -> None:
...
def main() -> int:
    ...
    try:
        ensure_profile_shape(repo_root, args.profile)
