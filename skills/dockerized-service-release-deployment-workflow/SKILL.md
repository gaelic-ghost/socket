...
8. For an asynchronous provider rollout, record the release tag, immutable digest, environment, deployment/run URL, and health target; reuse a live matching host-native continuation while the gate is pending and healthy, inspect those identities on wakeup, and only then decide whether to advance, diagnose, or roll back. Do not delete/recreate it after an unchanged snapshot; create/update only after it fires or becomes stale, with at least five minutes between rechecks.
