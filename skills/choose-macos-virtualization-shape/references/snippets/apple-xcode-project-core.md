...
- In an Apps/Packages workspace, declare shared Core modules as local Swift package products in each consuming app project's top-level `packages` map, then reference each required product from that app target's `dependencies`. A package declaration alone does not link code into a target. Use `projectReferences` only for genuine dependencies on another Xcode project's target; neither a group nor a folder reference is a module boundary.
