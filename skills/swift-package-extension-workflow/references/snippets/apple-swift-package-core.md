...
- For package plugins, macros, traits, generated source, or plugin permissions, use `swift-package-extension-workflow`; inspect Swiftly-selected and Xcode-selected toolchains separately when the package supports both environments.
- Keep package traits additive, make default-trait behavior explicit, and validate defaults, no-defaults, named traits, and meaningful combinations rather than testing only the default graph.
- Prefer deterministic build tool plugin commands with declared inputs and outputs; reserve prebuild commands for output sets that cannot be predicted, and do not let build tool plugins modify package source.
